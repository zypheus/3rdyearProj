...
@endif
<div {{ $attributes->merge(['class' => 'relative overflow-hidden bg-white rounded-2xl shadow-sm border border-slate-200 p-6 card-hover']) }}>
    <div class="flex items-start justify-between">
        ...
        @if($hasIconSlot || $icon)
            <div class="flex-shrink-0 p-3 rounded-xl bg-gradient-to-br {{ $gradientClass }} text-white shadow-lg">
                @if($hasIconSlot)
    ...
    <!-- Decorative gradient -->
    <div class="absolute -bottom-2 -right-2 w-24 h-24 bg-gradient-to-br {{ $gradientClass }} opacity-5 rounded-full blur-2xl"></div>
</div>
@if($href)
</a>
@endif
