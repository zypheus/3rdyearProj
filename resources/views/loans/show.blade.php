    ...
    <x-slot name="header">
        <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
            <div class="flex items-center gap-3">
                <a href="{{ route('loans.index') }}" class="p-2 text-slate-400 hover:text-slate-600 hover:bg-slate-100 rounded-lg transition-colors">
                ...
                <div>
                    ...
                    <p class="mt-0.5 text-sm text-slate-500">{{ ucfirst($loan->loan_type) }} loan application</p>
                </div>
            </div>
            <div class="flex items-center gap-2">
                @if ($loan->isPending() && $loan->user_id === auth()->id())
                    <a href="{{ route('loans.edit', $loan) }}" class="inline-flex items-center gap-2 px-4 py-2 bg-amber-500 text-white rounded-xl text-sm font-medium hover:bg-amber-600 transition-colors">
                    ...
                    <x-card>
                        ...
                        <div class="flex flex-col sm:flex-row sm:justify-between sm:items-start gap-4 mb-6">
                            <div>
                                ...
                                <p class="text-sm text-slate-500 mt-1">Applied on {{ $loan->created_at->format('F d, Y') }}</p>
                            </div>
                        ...
                        <div class="grid grid-cols-2 gap-6">
                            <div class="p-4 bg-slate-50 rounded-xl">
                                ...
                                <dd class="mt-1 text-2xl font-bold text-slate-900">₱{{ number_format($loan->amount, 2) }}</dd>
                            </div>
                            @if ($loan->approved_amount)
                                <div class="p-4 bg-emerald-50 rounded-xl">
                                    <dt class="text-sm font-medium text-emerald-600">Approved Amount</dt>
                                    <dd class="mt-1 text-2xl font-bold text-emerald-700">₱{{ number_format($loan->approved_amount, 2) }}</dd>
                                </div>
                            @endif
                            <div class="p-4 bg-slate-50 rounded-xl">
                                <dt class="text-sm font-medium text-slate-500">Term</dt>
                                <dd class="mt-1 text-lg font-semibold text-slate-900">{{ $loan->term_months }} months</dd>
                            </div>
                            <div class="p-4 bg-slate-50 rounded-xl">
                                <dt class="text-sm font-medium text-slate-500">Interest Rate</dt>
                                <dd class="mt-1 text-lg font-semibold text-slate-900">{{ $loan->interest_rate }}% <span class="text-sm font-normal text-slate-500">per annum</span></dd>
                            </div>
                            @if ($loan->disbursement_date)
                                <div class="p-4 bg-indigo-50 rounded-xl col-span-2">
                                    <dt class="text-sm font-medium text-indigo-600">Disbursement Date</dt>
                                    <dd class="mt-1 text-lg font-semibold text-indigo-900">{{ $loan->disbursement_date->format('F d, Y') }}</dd>
                                </div>
                            @endif
                        </div>
                        ...
                        @if ($loan->purpose)
                            <div class="mt-6 pt-6 border-t border-slate-200">
                                <dt class="text-sm font-medium text-slate-500 mb-2">Purpose</dt>
                                <dd class="text-slate-700 bg-slate-50 p-4 rounded-xl">{{ $loan->purpose }}</dd>
                            </div>
                        @endif
                        ...
                        @if ($loan->isRejected() && $loan->rejection_reason)
                            <div class="mt-6 p-4 bg-red-50 border border-red-200 rounded-xl">
                                <div class="flex items-start gap-3">
                                    ...
                                    <div>
                                        <dt class="text-sm font-semibold text-red-800">Rejection Reason</dt>
                                        <dd class="mt-1 text-red-700">{{ $loan->rejection_reason }}</dd>
                                    </div>
                                </div>
                            </div>
                        @endif
                                ...
                                <div class="space-y-3">
                                    @foreach ($loan->payments as $payment)
                                        <div class="flex items-center justify-between p-4 bg-slate-50 rounded-xl">
                                            <div class="flex items-center gap-3">
                                                <div class="flex-shrink-0 w-10 h-10 rounded-lg {{ $payment->status === 'confirmed' ? 'bg-emerald-100' : ($payment->status === 'rejected' ? 'bg-red-100' : 'bg-amber-100') }} flex items-center justify-center">
                ...
                <div class="space-y-6">
                    ...
                    <x-card title="Applicant">
                        <div class="flex items-center gap-4">
                            <span class="inline-flex items-center justify-center w-14 h-14 rounded-full bg-gradient-to-br from-indigo-500 to-purple-600 text-white text-lg font-bold shadow-lg">
                                {{ strtoupper(substr($loan->user->name, 0, 1)) }}
                            </span>
                            <div>
                                <p class="font-semibold text-slate-900">{{ $loan->user->name }}</p>
                                <p class="text-sm text-slate-500">{{ $loan->user->email }}</p>
                            </div>
                        </div>
                        @if ($loan->user->phone)
                            <div class="mt-4 pt-4 border-t border-slate-200 flex items-center gap-2 text-sm text-slate-600">
                    ...
                    @if ($canReview)
                        <x-card title="Review Actions">
                            ...
                            @if ($loan->canBeReviewed())
                                ...
                                {{-- Reject Form --}}
                                <form method="POST" action="{{ route('loans.reject', $loan) }}" class="p-4 bg-red-50 rounded-xl border border-red-200">
                                    @csrf
                                    @method('PATCH')
                                    <h4 class="text-sm font-semibold text-red-800 mb-3">Reject Loan</h4>
                                    <div>
                                    ...
                                    <button type="submit" class="mt-3 w-full inline-flex justify-center items-center gap-2 px-4 py-2.5 bg-red-600 text-white rounded-lg font-medium text-sm hover:bg-red-700 transition-colors" onclick="return confirm('Are you sure you want to reject this loan?')">
                                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    ...
                    @if (auth()->user()->isAdminOrOfficer() && $loan->isApproved())
                        <x-card title="Disbursement">
                            <form method="POST" action="{{ route('loans.activate', $loan) }}">
                                @csrf
                                @method('PATCH')
                                <p class="text-sm text-slate-600 mb-4">Confirm disbursement of funds to activate this loan.</p>
                                <button type="submit" class="w-full inline-flex justify-center items-center gap-2 px-4 py-3 bg-gradient-to-r from-emerald-600 to-emerald-700 text-white rounded-xl font-medium hover:from-emerald-700 hover:to-emerald-800 transition-colors" onclick="return confirm('Confirm loan disbursement and activation?')">
                                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M17 9V7a2 2 0 00-2-2H5a2 2 0 00-2 2v6a2 2 0 002 2h2m2 4h10a2 2 0 002-2v-6a2 2 0 00-2-2H9a2 2 0 00-2 2v6a2 2 0 002 2zm7-5a2 2 0 11-4 0 2 2 0 014 0z"/>
                                    </svg>
                                    Activate & Disburse
                    ...
                    @if ($loan->isPending() && ($loan->user_id === auth()->id() || auth()->user()->isAdmin()))
                        <x-card>
                            <div class="p-4 bg-red-50 rounded-xl border border-red-200">
                                ...
                                <p class="text-xs text-red-600 mb-3">This action cannot be undone.</p>
                                <form method="POST" action="{{ route('loans.destroy', $loan) }}" onsubmit="return confirm('Are you sure you want to delete this loan application?')">
                                    @csrf
                                    @method('DELETE')
                                    <button type="submit" class="w-full inline-flex justify-center items-center gap-2 px-4 py-2.5 bg-red-600 text-white rounded-lg font-medium text-sm hover:bg-red-700 transition-colors">
                                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
